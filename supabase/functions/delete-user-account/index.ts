...
const allowedOrigins = [
  ...
  'https://rms-9r5t4z5oz-felixs-projects-3080273c.vercel.app',
  'http://localhost:3000',
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (!userToken) {
      return new Response(JSON.stringify({
        error: 'Authentication error: Missing auth token'
      }), {
        headers: {
          ...commonCorsHeaders,
          'Content-Type': 'application/json'
    ...
    if (userError || !user) {
      ...
      return new Response(JSON.stringify({
        error: userError?.message || 'User not authenticated or token invalid'
      }), {
        headers: {
          ...commonCorsHeaders,
          'Content-Type': 'application/json'
