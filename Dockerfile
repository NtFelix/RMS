...
# Pins Server Action IDs across deploys so stale clients don't hit
# "Failed to find Server Action". MUST be the same value at build AND runtime and
# stable across deploys. Generate once with: openssl rand -base64 32
...
ARG ROBOTS_INDEXING=true
ENV ROBOTS_INDEXING=$ROBOTS_INDEXING
# Server decrypts bound Server Action arguments at runtime, so the key must be
# present here too and match the value used at build time.
ARG NEXT_SERVER_ACTIONS_ENCRYPTION_KEY
ENV NEXT_SERVER_ACTIONS_ENCRYPTION_KEY=$NEXT_SERVER_ACTIONS_ENCRYPTION_KEY
