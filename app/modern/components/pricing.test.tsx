...
describe('Pricing Component', () => {
  ...
  it('disables buttons and shows loading text when isLoading is true', () => {
    render(<Pricing onSelectPlan={mockOnSelectPlan} isLoading={true} />);

    // All buttons should now say "Processing..."
    const getStartedButtons = screen.getAllByText('Processing...');
    // Ensure we find a button for each plan (assuming 3 plans)
    expect(getStartedButtons.length).toBe(3);

    getStartedButtons.forEach(button => {
      expect(button).toBeInTheDocument();
      // Check if the button element itself is disabled
