...
const TenantPaymentBento = nextDynamic(() => import("@/components/tenants/tenant-payment-bento").then(mod => mod.TenantPaymentBento), {
    ssr: false,
    loading: () => <div className="h-full w-full animate-pulse bg-muted rounded-[2rem]" />
});
const RevenueExpensesChart = nextDynamic(() => import("@/components/charts/revenue-expenses-chart").then(mod => mod.RevenueExpensesChart), {
    ssr: false,
    loading: () => <div className="h-full w-full animate-pulse bg-muted rounded-[2rem]" />
});
const OccupancyChart = nextDynamic(() => import("@/components/charts/occupancy-chart").then(mod => mod.OccupancyChart), {
    ssr: false,
    loading: () => <div className="h-full w-full animate-pulse bg-muted rounded-[2rem]" />
});
const MaintenanceDonutChart = nextDynamic(() => import("@/components/charts/maintenance-donut-chart").then(mod => mod.MaintenanceDonutChart), {
    ssr: false,
    loading: () => <div className="h-full w-full animate-pulse bg-muted rounded-[2rem]" />
});
const NebenkostenChart = nextDynamic(() => import("@/components/charts/nebenkosten-chart").then(mod => mod.NebenkostenChart), {
    ssr: false,
...
interface DashboardClientProps {
    summary: {
        ...
        jaehrlicheAusgaben: number;
    };
    nebenkostenData: {
        nebenkostenChartData: any;
        nebenkostenYear: number;
    };
}

export default function DashboardClient({ summary, nebenkostenData }: DashboardClientProps) {
    const { nebenkostenChartData, nebenkostenYear } = nebenkostenData;
