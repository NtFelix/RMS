...
export default function LandingPage() {
  ...
  const handleSelectPlan = async (priceId: string) => {
    ...
    if (
      ...
      // Removed: && userProfile.stripe_price_id === priceId
    ) {
      // User has an active subscription, redirect to customer portal for any plan click
      await redirectToCustomerPortal(); // This function handles isProcessingCheckout internally
    } else {
      // User does not have an active subscription, or is not logged in.
      // Proceed with normal checkout flow or login prompt.
      setIsProcessingCheckout(true); // Set true before calling handleAuthFlow
      await handleAuthFlow(priceId);
      // handleAuthFlow (or proceedToCheckout within it, or if user not logged in)
      // is responsible for setting isProcessingCheckout to false.
    }
  };
