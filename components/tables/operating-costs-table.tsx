...
import { Nebenkosten, Mieter, Zaehler, Rechnung, Haus } from "@/lib/data-fetching" // Adjusted path, Removed ZaehlerFormData
import { OptimizedNebenkosten, AbrechnungModalData } from "@/types/optimized-betriebskosten"; // Removed ZaehlerModalData
import { isoToGermanDate } from "@/utils/date-calculations"
import { Edit, Trash2, FileText, Gauge, ChevronsUpDown, ArrowUp, ArrowDown, Calendar, Building2, Euro, Calculator, MoreVertical, X, Download, Pencil, Loader2 } from "lucide-react"
...
const ZaehlerVerwaltungModal = dynamic(() => import('@/components/meters/zaehler-verwaltung-modal').then(mod => mod.ZaehlerVerwaltungModal), { ssr: false })
import { ZaehlerAblesungenModal } from "@/components/meters/zaehler-ablesungen-modal";

import {
  getAbrechnungModalDataAction,
  deleteNebenkosten as deleteNebenkostenServerAction,
  bulkDeleteNebenkosten
} from "@/app/betriebskosten-actions" // Removed old zaehler imports
...
export function OperatingCostsTable({
  ...
  // Old openZaehlerModalOptimized removed - now using new ZaehlerAblesenModal
  ...
  // Old handleOpenZaehlerModal function removed - now using new ZaehlerAblesenModal

  // Old handleSaveZaehler function removed - now using new ZaehlerAblesenModal
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        console.debug('[OperatingCostsTable] getAbrechnungModalDataAction success', {
          ...
          zaehlerCount: Array.isArray(result.data.zaehler) ? result.data.zaehler.length : 'n/a',
          zaehlerAblesungenCount: Array.isArray(result.data.zaehler_ablesungen) ? result.data.zaehler_ablesungen.length : 'n/a',
    ...
    <div className="rounded-lg">
          ...
          <Table className="min-w-full">
            <TableHeader>
              <TableRow className="bg-gray-50 dark:bg-[#22272e] dark:text-[#f3f4f6] hover:bg-gray-50 dark:hover:bg-[#22272e] transition-all duration-200 ease-out transform hover:scale-[1.002] active:scale-[0.998] [&:hover_th]:[&:first-child]:rounded-tl-lg [&:hover_th]:[&:last-child]:rounded-tr-lg">
                ...
                <TableHeaderCell sortKey="zaehlerkosten" className="w-[130px] dark:text-[#f3f4f6]" icon={Gauge}>Zählerkosten</TableHeaderCell>
      ...
      {selectedNebenkostenForAbrechnung && abrechnungModalData && (
        <AbrechnungModal
          ...
          zaehler={abrechnungModalData.zaehler ?? []}
          zaehler_ablesungen={abrechnungModalData.zaehler_ablesungen ?? []}
          ownerName={ownerName}
          ownerAddress={(() => {
      ...
      {selectedNebenkostenForAblesen && (
        <ZaehlerAblesungenModal
          isOpen={isZaehlerAblesenOpen}
          onClose={() => setIsZaehlerAblesenOpen(false)}
          hausId={selectedNebenkostenForAblesen.haeuser_id}
          hausName={selectedNebenkostenForAblesen.Haeuser?.name || ''}
