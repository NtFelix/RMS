...
export function SelectableTenantDataTable() {
  ...
  const allIds = tenantData.map(tenant => tenant.id)

  const getAffectedItemsPreview = (selectedIds: string[]) => {
    return selectedIds.map(id => {
      const tenant = tenantData.find(t => t.id === id)
      return tenant ? `${tenant.tenant} (${tenant.apartment})` : id
    })
  }
              ...
              <Table>
                ...
                <TableBody>
                  {tenantData.length > 0 ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={7} className="text-center py-4">Keine Mietdaten verfügbar</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
