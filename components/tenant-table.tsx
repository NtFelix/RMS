...
type SortKey = keyof Tenant | 'wohnungName';
  ...
  const sortedAndFilteredData = useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(t =>
        ...
        (t.wohnung_id && wohnungsMap[t.wohnung_id]?.toLowerCase().includes(q))
      );
    }
    ...
    if (sortKey) {
      result.sort((a, b) => {
        ...
        if (sortKey === 'wohnungName') {
          ...
          valA = a[sortKey as keyof Tenant];
          valB = b[sortKey as keyof Tenant];
        }
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHeaderCell sortKey="wohnungName">Wohnung</TableHeaderCell>
        ...
        <TableBody>
          {sortedAndFilteredData.length === 0 ? (
            <TableRow>
              <TableCell colSpan={5} className="h-24 text-center">Keine Mieter gefunden.</TableCell>
            </TableRow>
          ) : (
            sortedAndFilteredData.map((tenant) => (
              <TenantContextMenu key={tenant.id} tenant={tenant} onEdit={() => onEdit?.(tenant)} onRefresh={() => router.refresh()}>
                <TableRow className="hover:bg-gray-50 cursor-pointer" onClick={() => onEdit?.(tenant)}>
                  ...
                  <TableCell>
                    {tenant.nebenkosten && tenant.nebenkosten.length > 0
                      ? tenant.nebenkosten.slice(0, 3).map(n => `${n.amount} €`).join(', ') + (tenant.nebenkosten.length > 3 ? '...' : '')
                      : '-'}
                  </TableCell>
                </TableRow>
              </TenantContextMenu>
